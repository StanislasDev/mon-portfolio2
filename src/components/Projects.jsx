    ...
    <div className="max-w-7xl mx-auto px-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between mb-16 gap-8">
        <div>
          ...
          <h3 className="text-4xl md:text-6xl font-black tracking-tighter">L'EXCELLENCE<br />EN INGÉNIERIE.</h3>
        </div>
      ...
      <div className="grid gap-8 md:grid-cols-2 lg:grid-cols-3">
        {projects.map((project, index) => (
          <motion.article 
            ...
            viewport={{ once: true }}
            className="group relative rounded-3xl overflow-hidden bg-white/5 border border-white/10 hover:border-cyan-500/50 transition-all duration-500"
          >
            {/* Image Container */}
            ...
            <div className="p-8">
              <div className="flex items-center gap-2 mb-4">
                {project.stack.map(tech => (
                  <span key={tech} className="text-[9px] uppercase tracking-widest px-2 py-1 bg-white/5 rounded-md text-white/60">
                    {tech}
                  </span>
                ))}
              </div>
              
              <h4 className="text-2xl font-bold tracking-tight mb-2 group-hover:text-cyan-400 transition-colors">
                {project.title}
              </h4>
              ...
              <button className="text-[10px] uppercase tracking-[0.2em] font-bold flex items-center gap-2 group/btn">
                Voir l'étude de cas
                <span className="w-8 h-[1px] bg-white/20 group-hover/btn:w-12 group-hover/btn:bg-cyan-500 transition-all duration-300"></span>
              </button>
            </div>
          </motion.article>
        ))}
